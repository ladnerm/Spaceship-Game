...
pub fn astroid_movement(
    mut astroids: Query<(&mut Transform, &Astroid)>,
    time: Res<Time>
) {
...
pub fn despawn_astroid(
    mut commands: Commands,
    query: Query<(Entity, &Transform), With<Astroid>>,
) {
...
pub fn tick_astroid_timer(
    mut astroid_spaw_timer: ResMut<AstroidSpawnTimer>, 
    time: Res<Time>
) {
...
pub fn rotate_astroid(
    time: Res<Time>, 
    mut query: Query<(&Astroid, &mut Transform)>
) {

    for (astroid, mut transform) in query.iter_mut() {
        transform.rotation *= Quat::from_rotation_z(astroid.rotate_direction * time.delta_seconds());
    }
}